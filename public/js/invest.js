...
function displayFarm(farm,id){
	...
	var investurl = '/investor/invest/'+username+'/'+farm.name.replace(/ /g,'-');
	var form = $('<form></form>').attr('method','post').attr('action',investurl);
	var button = $('<button>Invest</button>').attr('type','submit');
	form.append(button);
	div.append(form);
	...
	$.ajax({
		...
		success: function(farms){
			...
			for(var farm in farms){
				if(!farms[farm].hasOwnProperty("investor")){
					noInvestors.push(farms[farm]);
				}
			}
			for(var invest in noInvestors){
				displayFarm(noInvestors[invest],investId);
			}
		}
	});
});