...
function displayFarm(farm,id){
	...
	var capacity = ' '+farm.capacity+' units'; var investor = ' Investor: '+farm.investor;
	...
	var user1 = $('<i></i>').attr('class','fa fa-user');
	var p = $('<p></p>'); var p5 = $('<p></p>'); var heading = $('<h6></h6>').attr('class','heading');
	var p1 = $('<p></p>'); var p2 = $('<p></p>'); var p3 = $('<p></p>'); var p4 = $('<p></p>'); var p6 = $('<p></p>');
	if(type == 'poultry'){
	...
	p6.append(user1).append(investor);
	div.append(heading).append(p).append(p1).append(p2).append(p3).append(p4).append(p5).append(p6);
	li.append(img).append(div);
	id.append(li);
}
$(document).ready(function(){
	$('#dashboard').attr('href','/investor/'+url);
	var href = '/investor/'+'invest/'+url;
	...
	// this is used to display the farm thats owned by the investor
	$.ajax({
		type: 'GET',
		url: 'farmowned/'+ username,
		success: function(farms){
			for(var farm in farms){
				displayFarm(farms[farm],farmId);
			}
		}
	});
});